...
\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto] 
 ...
 \node[state] (s_ab)[below=of s_a] {ab};
 \path[->] 
 ...
 (s_a) edge [bend right,left] node {b} (s_ab)
 (s_ab) edge [bend right,right] node {b} (s_a)
 (s_b) edge [bend right,below]node {a} (s_ab)
 (s_ab) edge [bend right,above]node {a} (s_b);
\end{tikzpicture} 
 \end{document}